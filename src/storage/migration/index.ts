 ...
 * record axis is a `recordKind -> {version, path}` map). The registry ships
 * EMPTY — no real historical steps — and the transactional engine is
 * parameterized over an abstract {@link MigrationTarget}, so it neither touches a
 * real Home nor hardcodes any domain derived-state list. Real wiring (the
 * canonical rebuild/validate boundary, the real target) is injected elsewhere.
 */
