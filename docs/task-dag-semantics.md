...
2. 每个非终态 WorkItem 由边分类集合**确定性投影**为：`ready` / `blocked` / `unreleasable`（叠加 `input-waiting` 传播标记）。
...
6. 动态修订（加节点、加边、删边、退役）只允许在**未释放**（`pending`）的 WorkItem 上改边；每次变更重跑无环与存在性校验。
...
| 输入等待 | `InputRequest.blockedRefs` 可指向 WorkItem；存储层校验引用存在；**无沿依赖边的传递传播** | `src/storage/taskStore.ts:3937` |
...
2. **投影纯函数**：给定同一持久快照 `(dependsOn, status, disposition, open InputRequests)`，投影输出唯一确定，与时间、派发顺序、观察者无关。
3. **表达边界**：DAG 只表达语义与产物依赖；**不表达** Agent、Session、Provider 并发、文件锁或资源配额（属 Layer 2 与 T6 Resource Broker）。
4. **Task 局部性**：依赖边只在同一 Task 聚合内有效。Yui 无 Task-to-Task 原生依赖字段；跨 Task 依赖只记录在 Description/Brief（如本 Task 的 T0=task-27），不创建伪 WorkItem 依赖。
5. **AND 语义**：一个 WorkItem 的释放要求**全部**边满足。OR-join 不可表达——需要 OR 的场景由 Leader 用动态修订（§6）或退役建模。
...
叠加标记（可与任一 blocked 投影共存）：

- `input-waiting`：该 WorkItem 被某个 open `InputRequest.blockedRefs` 指向，或其任一依赖边（传递）指向 input-waiting 节点。投影须给出到 InputRequest 的传递链。

### 3.4 确定性与单调性

- 投影只随持久状态变化重算；不随时间、不随观察次数变化。
...
1. WorkItem 在其全部依赖边 `satisfied` 时进入 `ready`；`ready` 是派生状态，不写回。
...
1. `blocked` 投影必须携带**传递根因链**，而不仅是直接依赖：v  blocked on u，u blocked on w（`failed-open`）→ v 的阻塞显示 `v → u → w (failed)`。现状 CLI 只显示直接依赖（`taskOverviewCommand.ts:386`），契约要求传递化。
...
| 依赖被 open InputRequest 阻塞 | `active` + `input-waiting` 链 | `blocked (waiting, input-waiting)` | 回答 InputRequest → 重算投影 |

**不自动级联**：依赖失败/退役**不自动**使下游失败或取消。下游保持 `pending`，投影显式标出原因；所有解除动作都是 Leader 的显式持久决议。理由：失败是否可恢复、下游是否仍有价值，是语义判断，属 Leader 职责（T0 §0、§3.1）。

### 5.2 替换重定向（投影层）

1. `retired` 带 `replacementWorkItemId = R'` 时，依赖边的满足性**跟随替换链**：边 `v → R` 满足当且仅当链 `R → R' → ...` 到达 `completed`。
2. **存储的 `dependsOn` 永不改写**——重定向是投影规则，不是数据迁移。审计轨迹保留原始边。
3. 替换链必须无环；带替换的退役在写入时校验链无环（新增校验，与 `assertAcyclicWorkItems` 同层）。链长有限。
4. 若 `R'` 在退役时已 `completed`，下游在下一次投影中立即 `ready`——确定、无窗口期歧义。
5. 替换不传递语义义务之外的东西：R' 是否承接 R 的意图，由 Leader 在退役 disposition 中记录；投影只做状态跟随。
...
2. **加边 `v → u`**：v 必须 `pending`；u 必须存在且同 Task；结果图必须无环（存储层既有校验覆盖）。满足后 v 的投影按新边重算（可使 `ready → blocked`）。
...
| 约束 | 规则 | 现状/新增 |
|---|---|---|
| 无环 | 依赖图必须无环；任何写入（创建、改边、替换）后重算 | 现状：`assertAcyclicWorkItems`（`taskStore.ts:4238`），写入与加载双校验 |
...
| 替换链无环 | `replacementWorkItemId` 链必须无环且有限 | **新增**（与无环同层校验；本契约定义语义，实现属后续 Task） |
...
3. **传递根因链**：阻塞展示必须沿依赖边回溯到根因（失败/退役/输入等待/活跃执行的祖先），而非仅列直接依赖。现状缺口：CLI 只列直接依赖（`taskOverviewCommand.ts:386`）、投影只产 "waiting on a dependency"（`taskExecutionProjection.ts:727`）。
4. **输入等待链**：input-waiting 展示给出 WorkItem → ... → InputRequest 的链；回答输入后投影重算。
5. **跳过/替换可见**：退役节点展示 disposition（跳过原因、替换指针）；替换重定向在下游阻塞原因中可见（`v → R (retired) → R'`）。
6. **DAG 视图**（T9 实现）：节点 = WorkItem，边 = `dependsOn`，节点状态 = 投影，边可按分类标注（satisfied/active/failed-open/dead）。视图是投影的渲染，不是独立数据源；不为表格展示改变领域语义（D-09）。
...
2. **改边命令**：`pending` WorkItem 的加边/删边命令与持久化（含 revision 与审计），替换链无环校验的存储层实现（Yui Core 实现 Task）。
